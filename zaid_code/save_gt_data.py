...
psData = np.zeros(600)
psBPMData = np.zeros(600)
psPsData=np.zeros(600)
t = np.zeros(600)

Rimage=np.zeros((480,640,600),dtype=np.uint8)
...
#                            # (see also: np.roll)
#        camData[-1] = signal
        t[self.ctr] = (datetime.now() - start_time).total_seconds()
      
#        t[-1] = (datetime.now() - start_time).total_seconds()
        if self.ctr>0:
            fps=1/(t[self.ctr]-t[self.ctr-1])
            print(fps)
...
#                            # (see also: np.roll)
        psData[self.ctr] = data[0][0]

#        psBPMData[:-1] = psBPMData[1:]
        psBPMData[self.ctr] = data[0][2]
        
        #Pulse oximetry
#        camBPMData[:-1] = camBPMData[1:]
        psPsData[self.ctr] = data[0][3]
        
        
        if self.ctr==599:
            print("save condition met")
            np.savez('with_gt_mat180.npz',Rimage=Rimage,t=t,psData=psData,psBPMData=psBPMData,psPsData=psPsData)
            print("saved my lord")
#            break
        self.ctr=self.ctr+1
        print(self.ctr)
...
with picamera.PiCamera(resolution='640x480') as camera:
    # Fix the camera's white-balance gainscamera.iso = 150
